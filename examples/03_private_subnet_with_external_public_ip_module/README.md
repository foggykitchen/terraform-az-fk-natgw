# Example 03: Private Subnet with NAT Gateway and External Public IP Module
...
This example keeps the baseline architecture from Example 01, but moves **Public IP ownership**
into a separate `terraform-az-fk-public-ip` module and then injects that Public IP into the NAT Gateway.
...
<img src="03-private-subnet-with-external-public-ip-module-architecture.png" width="900"/>

*Figure 1. Private subnet with NAT Gateway providing outbound Internet access for private VMs, using a Public IP created by a dedicated Public IP module.*
...
- An external **Azure Public IP** via `terraform-az-fk-public-ip`
- An **Azure NAT Gateway** consuming that existing Public IP via `terraform-az-fk-natgw`
...
- how to keep **Public IP lifecycle ownership separate** from NAT Gateway lifecycle,
...
- The NAT Gateway uses a **Public IP created by a dedicated Public IP module**
- All outbound traffic uses that **NAT Gateway public IP**
- Inbound access is **not exposed to the Internet**
- Operator access can be provided via **Azure Bastion tunneling**

<img src="03-nat-gateway-portal-outbound-ip.png" width="900"/>

*Figure 2. NAT Gateway using an externally managed Public IP for outbound traffic from the private subnet.*
...
This example also validates the module contract:
...
*Figure 3. NAT Gateway outbound IP configuration in Azure Portal, backed by a separately managed Public IP resource.*

### NAT Gateway – Subnet Association

<img src="03-nat-gateway-portal-subnet-association.png" width="900"/>
